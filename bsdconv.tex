...
(d0)--(d1)  node  [black,midway,yshift=-1cm] { inter };
	...
	\section{Type \& Flag}
		...
		\subsection{Flag}
			...
			\paragraph{}
			The code point packets structure is retained, including flags, within cascaded conversions, but not for shell pipe. Figure 11 demonstrate the flow of conversion "ESCAPE:UNICODE,BYTE$|$SKIP,UTF-8:UTF-8".
				\begin{figure}[H]
					\centering
					\begin{tikzpicture}[auto,node distance=0.5cm,on grid=false]
						...
						\node (table){\begin{tabular}{|c|c|c|}
					    	...
					    	\hline
						    \end{tabular}};
					    \node [below=1 of table,rectangle,draw] (input) {\%u03B1\%CE\%B2};
					    \node [below=0 of input] {Input (UTF-8 literal)};
