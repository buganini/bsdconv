...
DPRINTF("FROM");
	...
	while(ins->feed_len){
DPRINTF("%d:%d", __LINE__,ins->feed_len);
...
DPRINTF("INTER %d: %d", __LINE__, ins->inter_data.data);
		...
		DPRINTF("INTER_DATA.P=%d", ins->inter_data.p);
...
DPRINTF("P%d", __LINE__);
				...
				break;
			case MATCH:
DPRINTF("inter_match %d", __LINE__);
